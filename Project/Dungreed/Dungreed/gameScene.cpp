...
{
	IMAGEMANAGER->addImage("맵", "image/gameScene/던그리드_마을_지형_이미지.bmp", BACKDCSIZEX, BACKDCSIZEY, true, RGB(255, 0, 255));
	map = IMAGEMANAGER->findImage("맵");
	map->setX(0);
	map->setY(0);

	IMAGEMANAGER->addImage("충돌", "image/gameScene/던그리드_마을_지형_충돌박스.bmp", BACKDCSIZEX, BACKDCSIZEY, true, RGB(255, 0, 255));
...
void gameScene::render()
{
	...
	IMAGEMANAGER->findImage("충돌")->render(getMemDC());
...
void gameScene::playerMove()
{
	...
	{
		_y -= 30;	// 플레이어 y좌표 감소
		if (_cameraVerticalMoveOk) _cameraY -= 30; // 카메라 수직이동이 가능한 상태라면 카메라 y좌표 감소
	}
